...
class _NFCScreenState extends State<NFCScreen> {
  ...
  Future<NFCResult> hasNFC() async {
    try {
      ...
      return NFCResult(
        success: true,
        message: isAvailable ? 'NFC khả dụng' : 'NFC không khả dụng',
      );
    } catch (e) {
      return NFCResult(success: false, message: 'Lỗi kiểm tra NFC: $e');
    }
  }

  Future<NFCResult> readNFC(
    List<String> mrz, {
    String alertMessageIos = 'Đưa thẻ CCCD gần thiết bị',
    NFCProgressCallback? onProgress,
  }) async {
    try {
      onProgress?.call(0.05, 'Đang kiểm tra NFC...');
      ...
      onProgress?.call(0.2, 'Đang kết nối NFC...');
      await nfc.connect(iosAlertMessage: alertMessageIos);

      onProgress?.call(0.3, 'Đang xác thực BAC...');
      await passport.startSession(dbaKey);

      // Đọc DG1
      onProgress?.call(0.5, 'Đang đọc DG1...');
      ...
      onProgress?.call(0.7, 'Đang đọc DG2...');
      try {
      ...
      return NFCResult(success: false, message: 'Lỗi đọc NFC: $e');
    }
  }
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: ListView(
          children: [
            const Text(
              'Nhập 3 dòng MRZ (từ mặt sau CCCD):',
    ...
    return Column(
      ...
      children: [
        ...
        if (result.dg1Data != null) ...[
          Text('🆔 DG1 (MRZ):', style: theme.textTheme.titleMedium),
          const SizedBox(height: 6),
          Text('Số CCCD: ${result.dg1Data!['documentNumber']}'),
          Text('Ngày sinh: ${result.dg1Data!['birthDate']}'),
          Text('Hết hạn: ${result.dg1Data!['expiryDate']}'),
        ],
        const SizedBox(height: 12),
        if (result.dg13Data != null) ...[
          Text('📄 DG13:', style: theme.textTheme.titleMedium),
          const SizedBox(height: 6),
